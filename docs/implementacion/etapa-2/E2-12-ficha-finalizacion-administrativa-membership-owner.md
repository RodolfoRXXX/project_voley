...
**CERRADA — implementación integrada y UAT aprobada.** Fecha de definición y aprobación documental: 2026-09-15. Implementación: `9624e1b`; integración técnica no fast-forward en `dev`: `75f33cc`; evidencia: `E2-12-informe-implementacion.md`; cierre: `E2-12-cierre.md`. El corte de autoridad previo al versionado original fue `dev == origin/dev == 191b3590479cad024a55db0e5d231a76438b0d7b`.
...
Revisión funcional posterior a UAT-03: E2-02 y Documento 5 consolidan que ownership, Persona y Membresía son conceptos independientes, que el Owner es el Usuario propietario del Grupo, que ownership no crea Membresía y que la ausencia de Persona no bloquea administración owner-scoped. No existe una regla superior que exija identidad deportiva para ejercer ownership. Por ello se corrige la exigencia inicial de Persona propia de E2-12: la Cuenta y el ownership autorizan; la Persona propia, si existe, se consulta exclusivamente para impedir autoobjetivo. Esta corrección no modifica AUD-C05, Documento 5 ni arquitectura.
...
Actor: UID autenticado con Cuenta canónica compatible y **ownership vigente** de un Grupo canónico activo. Sujeto: una Membresía activa de **otra Persona** en ese Grupo y su Temporada abierta exacta. La Persona propia del Owner es opcional y se consulta sólo para comprobar la prohibición de autoobjetivo. Si está ausente, el Owner puede administrar a un tercero sin crear Persona ni Membresía; si existe y coincide con la Persona objetivo, el backend devuelve `TARGET_IS_SELF` y para finalizar la Membresía propia se usa E2-10. Un Owner no necesita Membresía propia para administrar terceros.

Un integrante común, un global admin, un claim, `users.roles`, `memberIds`, `adminIds`, `admins`, un BFF legacy o conocer `membershipId` no autoriza. Sólo existe un Owner vigente: no se introduce “otro Owner” ni rol nuevo. La propiedad se revalida junto con la raíz en la transacción; una transferencia antes del commit hace fallar al Owner anterior, y una transferencia posterior no revierte el cierre. Un retry confirmado sólo recupera el resultado para el mismo actor si conserva Cuenta y acceso vigente al Grupo; la Persona propia actual se reconsulta únicamente para impedir `TARGET_IS_SELF`. Si perdió ownership responde `GROUP_NOT_ACCESSIBLE`, sin escritura ni exposición del efecto. El intent histórico queda intacto. Un nuevo Owner puede iniciar su **propia** decisión con clave nueva sobre una activación que siga activa; no adopta el intent anterior.

| Actor/contexto | Persona propia | Resultado |
| --- | --- | --- |
| Owner vigente con Cuenta compatible | ausente | puede preparar y finalizar una Membresía válida de tercero |
| Owner vigente con Cuenta compatible | presente y distinta del objetivo | puede preparar y finalizar |
| Owner vigente con Cuenta compatible | presente y coincidente con el objetivo | `TARGET_IS_SELF`; usar E2-10 |
| No Owner o ex Owner | ausente o presente | `GROUP_NOT_ACCESSIBLE`; Persona no concede autoridad |
...
`prepare` es read-only, revalida Cuenta, Grupo/ownership, Persona propia opcional, Membresía, Persona objetivo, Temporada, guards y período de la activación actual, y devuelve nombre/apellido canónicos más una referencia opaca. `activationRef` es el SHA-256 con dominio `sportexa:E2-12:administrative-finalization-activation-ref:v1` del encoding length-prefixed de actor UID, Grupo, Membresía, Persona objetivo, Temporada, ordinal, ID del período vigente y versión del active guard. La Persona propia no integra el digest porque no autoriza ni identifica la activación; se relee separadamente para `TARGET_IS_SELF`. El digest no expone esos valores. No es firma de autoridad: un cliente capaz de conocerlo o fabricarlo sigue sin poder omitir ninguna validación backend. Si cualquier correlación de la activación cambia entre `prepare` y `finalize`, el backend devuelve `MEMBERSHIP_ACTIVATION_CHANGED` sin cerrar la nueva activación. Una nueva reactivación exige repetir `prepare`, mostrar la nueva confirmación humana y generar clave nueva. La referencia no se añade al DTO cerrado E2-11 ni modifica su reader.
...
La primera ejecución lee el intent determinista y revalida Cuenta, Grupo activo y `ownerId == uid` dentro de la transacción. Después resuelve el vínculo Cuenta–Persona de forma opcional: ausencia es válida; vínculo o Persona declarados pero incompatibles fallan cerrados. Si no existe un intent terminal correlacionado, lee la raíz por el `membershipId` recibido como **referencia**, comprueba `groupId` exacto y, sólo cuando existe Persona propia, que `personId` objetivo sea distinto. Valida Persona objetivo canónica compatible y `seasonId` de la raíz, no del cliente. La Persona objetivo ausente o incompatible impide una nueva finalización; la política tolerante de presentación `UNAVAILABLE` de E2-11 no autoriza una escritura. Antes de invocar CU-027 recomputa `activationRef` desde la activación vigente autoritativa y exige igualdad con la referencia confirmada; un token viejo jamás selecciona el ordinal nuevo. `finalize` vuelve a resolver la Persona propia actual: si se creó entre `prepare` y el commit y coincide con el objetivo, prevalece `TARGET_IS_SELF` y no se escribe.
...
| Persona propia creada entre `prepare` y `finalize` | se relee dentro de la transacción; si coincide con la Persona objetivo, `TARGET_IS_SELF` sin cierre ni intent; si es distinta, la decisión de tercero continúa |
...
| `UNAUTHENTICATED`, `ACCOUNT_REQUIRED` | autenticación o Cuenta canónica del actor insuficiente |
...
`PERSON_REQUIRED` no es un resultado de estas dos callables E2-12. Se conserva sin cambios para las capacidades de incorporación, pertenencia propia y salida propia que legítimamente requieren Persona.
...
**Autorización/contexto:** no autenticado; Cuenta ausente; Owner sin Persona propia válido; Owner con Persona distinta válido; Persona propia declarada pero incompatible falla cerrada; integrante no Owner; global admin sin ownership; Grupo inexistente/ajeno/inactivo; Owner sin Membresía propia válido; objetivo propio rechazado; Persona coincidente creada entre `prepare` y `finalize` rechazada con `TARGET_IS_SELF`; `membershipId` inexistente, de otro Grupo o Temporada; Persona objetivo ausente/incompatible; active guard de otra Temporada; open guard ausente/incompatible; Temporada exacta cerrada/cambiada. Se prueba que Grupo ajeno o raíz externa no revelan existencia y que la ausencia de Persona no crea Persona, Membresía ni autoridad sobre otro Grupo.
...
Cuentas Google existentes del Emulator: A, Owner de Grupo canónico sin Persona para UAT-03; B, integrante activo no Owner y Persona presentable; C, autenticado no Owner. Usar proyecto `demo-*`, Functions/Auth/Firestore Emulator y navegador local; no conectar Firebase remoto ni integrar la rama Auth/UAT aislada. Registrar IDs y clave del intento de prueba **sólo en evidencia local protegida** si se necesita replay; no publicarlos en UI o logs. Preparar B activo v3 en Temporada abierta, o fixture v1 específica para prueba técnica.

Recorrido **manual**: A, todavía sin Persona, abre `Integrantes`, ve a B y permanece fuera del roster salvo que posea una Membresía independiente. Abre “Finalizar Membresía” de B y `prepare` muestra el nombre canónico; cancela y verifica sin cambios; abre de nuevo y confirma; observa loading, éxito y roster reconsultado sin B. A continúa como Owner, sin Persona y fuera del roster. B reconsulta “Grupos que integrás” y ya no ve el Grupo. Inspección Emulator comprueba misma raíz finalizada, período cerrado, guard activo retirado, lifecycle/intent exactos con `actorPersonId: null`, Grupo/owner/Temporada y Solicitudes sin cambios, y ninguna Persona/Membresía creada para A. C intenta el callable y recibe `GROUP_NOT_ACCESSIBLE`; la UI común no ofrece el control. Un retry de A con la clave original recupera el mismo cierre sin duplicar transición. Verificar cancelación, error, foco, teclado, anuncios y responsive.
...
18. **Dado** A Owner vigente sin Persona propia y B tercero activo, **cuando** A prepara y confirma, **entonces** puede finalizar exclusivamente la Membresía de B sin crear Persona ni Membresía para A y sin aparecer en el roster.
19. **Dado** A sin Persona durante `prepare` y una Persona propia coincidente creada antes de `finalize`, **cuando** confirma, **entonces** la transacción relee la identidad actual y devuelve `TARGET_IS_SELF` sin escritura.
