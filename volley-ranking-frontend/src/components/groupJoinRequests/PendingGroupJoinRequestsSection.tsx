...
export function PendingGroupJoinRequestsSection({ groupId }: { groupId: string }) {
  ...
  const decide = async () => {
    ...
    markBusy(requestId, true); setError(""); setNotice(action === "approve" ? "Coordinando aprobación y Membresía…" : "Confirmando rechazo…");
    try {
      ...
      intents.current.confirm(action, requestId); setItems((current) => current.filter((candidate) => candidate.id !== requestId)); setNotice(action === "approve" ? "Aprobación y Membresía confirmadas." : "Rechazo confirmado; no se creó ninguna Membresía.");
    } catch (cause) {
