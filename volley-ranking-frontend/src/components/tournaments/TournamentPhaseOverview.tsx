...
export function TournamentPhaseOverview({ metrics, phaseSnapshot, tournamentStatus }: TournamentPhaseOverviewProps) {
  const isFinalized = tournamentStatus === "finalizado";
    ...
    <section className={`rounded-xl border bg-white p-5 space-y-4 ${isFinalized ? "border-emerald-200" : "border-neutral-200"}`}>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className={`text-sm font-medium ${isFinalized ? "text-emerald-700" : "text-orange-600"}`}>
            {isFinalized ? "Resumen final" : "Resumen de fase"}
          </p>
          <h2 className="text-lg font-semibold text-neutral-900">{phaseLabel}</h2>
          <p className="text-sm text-neutral-600">
            {isFinalized ? "Estado definitivo del torneo." : `Estado actual: ${phaseStatus}.`}
          </p>
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center justify-between text-xs text-neutral-500">
            <span>{isFinalized ? "Resultados cerrados" : progressLabel}</span>
            <span>{isFinalized ? "100% completo" : `${progress}% completo`}</span>
          </div>
      </div>
