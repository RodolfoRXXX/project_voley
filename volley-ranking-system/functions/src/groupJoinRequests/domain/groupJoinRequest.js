...
function freezeRequest(data) {
  ...
  Object.defineProperty(request, "approveAfterMembership", {
    enumerable: false,
    value({ decisionIntentId, decidedBy, decidedAt, membershipId }) {
      if (this.estado !== "pendiente") throw new InvalidGroupJoinRequestStateError("Request is not pending");
      const timestamp = requireTimestamp(decidedAt, "Decision timestamp");
      if (timestamp.toDate().getTime() < this.createdAt.toDate().getTime()) throw new InvalidGroupJoinRequestStateError("Decision precedes creation");
      return freezeRequest({
        ...
        membershipId: requireId(membershipId, "Membership id"), schemaVersion: 2,
      });
    },
  });
...
function hydrateGroupJoinRequest(requestId, data) {
  ...
  if (!pending && !cancelled && !rejected && !approved) throw new InvalidGroupJoinRequestStateError("Request state is incompatible");
  exactKeys(data, pending ? PENDING_FIELDS : cancelled ? CANCELLED_FIELDS : rejected ? REJECTED_FIELDS : APPROVED_FIELDS);
  ...
  if (approved) requireId(data.membershipId, "Membership id");
  return freezeRequest({ requestId, ...data });
}

module.exports = { APPROVED_FIELDS, CANCELLED_FIELDS, PENDING_FIELDS, REJECTED_FIELDS, InvalidGroupJoinRequestStateError, buildGroupJoinRequest, hydrateGroupJoinRequest };
