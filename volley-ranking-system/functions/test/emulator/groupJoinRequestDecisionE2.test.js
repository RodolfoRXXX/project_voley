...
test("E2-07 decide Solicitudes con coordinación recuperable y cardinalidad autoritativa", async (t) => {
  ...
  try {
    ...
    await t.test("lifecycle finalizado v2 reactiva la misma Membresía y materializa sus períodos", async () => {
      ...
      const state = await cardinality(groupId, requestId); assert.equal(state.active.size, 1); assert.equal(state.claims.size, 0); assert.equal(state.requests.docs[0].data().estado, "aprobada"); const lifecycle = (await db.collection("membershipLifecycleGuards").doc(lifecycleId).get()).data(); assert.equal(lifecycle.lifecycleGuardVersion, 3); assert.equal(lifecycle.rootState, "active"); assert.equal(lifecycle.membershipId, membershipId); assert.equal(lifecycle.lastActivationOrdinal, 2);
    });
    ...
    await t.test("payloads, ownership y efectos colaterales quedan acotados", async () => {
      ...
      for (const actor of [outsider, globalAdmin, candidate]) assert.equal((await call("rejectGroupJoinRequest", { groupId, requestId, idempotencyKey: key }, actor)).body.error.details.reason, "GROUP_NOT_ACCESSIBLE");
