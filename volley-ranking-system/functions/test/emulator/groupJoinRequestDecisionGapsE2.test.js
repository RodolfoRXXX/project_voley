...
test("E2-07 closes the remaining deterministic persistence frontiers", async (t) => {
  ...
  try {
    ...
    await t.test("ownership before claim and finalized membership keep authoritative semantics", async () => {
      ...
      assert.equal((await call("approveGroupJoinRequest", { groupId, requestId, idempotencyKey: key }, owner)).body.error.details.reason, "GROUP_NOT_ACCESSIBLE");
      ...
      await db.runTransaction(async (transaction) => { const [snapshot, periodSnapshot] = await transaction.getAll(membershipRef, periodRef); transaction.update(membershipRef, { ...snapshot.data(), estado: "finalizada", fechaEgreso: finalizedAt }); transaction.update(periodRef, { ...periodSnapshot.data(), estado: "cerrado", endedAt: finalizedAt }); transaction.set(register("membershipLifecycleGuards", membershipLifecycleGuardId(groupId, personId)), { membershipId, personId, groupId, seasonId: active.seasonId, rootState: "finalized", lastActivationOrdinal: active.activationOrdinal, finalizedAt, lifecycleGuardVersion: 3 }); transaction.delete(activeRef); });
