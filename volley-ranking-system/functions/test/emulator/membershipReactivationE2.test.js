...
test("E2-09 reactiva Membresías con historia durable y coordinación recuperable", async (t) => {
  ...
  try {
    ...
    await t.test("Temporada sin apertura vigente consume outcome estable y el retry no reactiva", async () => {
      ...
      const first = await call("approveGroupJoinRequest", { groupId, requestId, idempotencyKey: key }); assert.equal(first.body.error.details.reason, "OPEN_SEASON_REQUIRED"); const intentRef = db.collection("groupJoinRequestDecisionIntents").doc(groupJoinRequestDecisionIntentId(owner.uid, key)); assert.equal((await intentRef.get()).exists, false); assert.equal((await db.collection("groupJoinRequestApprovalCoordinations").doc(requestId).get()).exists, false);
      const retry = await call("approveGroupJoinRequest", { groupId, requestId, idempotencyKey: key }); assert.equal(retry.body.error.details.reason, "OPEN_SEASON_REQUIRED"); assert.equal((await db.collection("memberships").doc(membershipId).get()).data().schemaVersion, 2); assert.equal((await db.collection("memberships").doc(membershipId).collection("validityPeriods").get()).size, 0);
    });
    ...
    await t.test("cambio concurrente del guard de Temporada converge sin confirmar contexto stale", async () => {
      ...
      else { assert.equal(approval.reason?.reason, "OPEN_SEASON_REQUIRED"); assert.equal(root.schemaVersion, 2); assert.equal(periods.size, 0); assert.equal(request.estado, "pendiente"); assert.equal((await db.collection("groupJoinRequestDecisionIntents").doc(groupJoinRequestDecisionIntentId(owner.uid, key)).get()).exists, false); }
    ...
    await t.test("finalización entre unidades consume SUPERSEDED; misma clave no reabre y una nueva sí", async () => {
      ...
      const store = createFirestoreGroupJoinRequestStore({ db, groupCapability, seasonCapability, personCapability, membershipCapability: wrappedMembership, repository: createFirestoreGroupJoinRequestRepository({ db }) }); await assert.rejects(() => store.approve({ userId: owner.uid, groupId, requestId, idempotencyKey: key }), (error) => error?.reason === "GROUP_NOT_ACCESSIBLE"); assert.equal(membershipCalls, 1);
      await assert.rejects(() => store.approve({ userId: candidate.uid, groupId, requestId, idempotencyKey: key }), (error) => error?.reason === "MEMBERSHIP_REACTIVATION_SUPERSEDED"); assert.equal(membershipCalls, 2); await db.collection("groups").doc(groupId).update({ ownerId: owner.uid });
      ...
      await assert.rejects(() => store.approve({ userId: owner.uid, groupId, requestId, idempotencyKey: key }), (error) => error?.reason === "MEMBERSHIP_REACTIVATION_SUPERSEDED"); assert.equal(membershipCalls, 2); assert.equal((await db.collection("memberships").doc(membershipId).get()).data().periodCount, 2);
